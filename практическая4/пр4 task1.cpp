...
namespace
{
    ...
    void PrintGreeting(const std::wstring& city, WORD foregroundColor)
    {
        ...
        std::wcout << L"Данные сохранены. Ваш любимый город: " << city << L"\n";
...
int main()
{
    ...
    if (!hasCity)
    {
        std::wcout << L"Введите любимый город: ";
    ...
    if (!hasColor)
    {
        std::wcout << L"Введите цвет текста (0 - 15): ";
        std::wcin >> color;
        if (color > 15) color = 7; // безопасное значение (серый)
